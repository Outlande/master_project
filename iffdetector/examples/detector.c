...
int read_dir_r(char *path, char* res)
{
 DIR *dp = NULL;
 struct dirent *st;
 struct stat sta;
...
void test_detector_folder(char *datacfg, char *cfgfile, char *weightfile, char *foldername, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    char **names = get_labels(name_list);
    ...
    float nms=.45;
    int idx = 0;
    char res_folder[5000000] = "\0";
    read_dir_r(foldername, res_folder);
    int file_length = 0;
    for (int x=0; x < strlen(res_folder); x++) {
        if (res_folder[x] == ' ') {
            file_length++;
        }
    }
    ...
    int curr_num = 0;
    while(1){
        printf("Now processing: %d / %d\n", curr_num, file_length);
        int x = 0;
        while(res_folder[idx] != ' ') {
            filename[x] = res_folder[idx];
            x++;
            idx++;
        }
        idx++;
        ...
        //if (nms) do_nms_obj(boxes, probs, l.w*l.h*l.n, l.classes, nms);
        if (nms) do_nms_sort(dets, nboxes, l.classes, nms);
        int person_loc[nboxes*4];
        int person_num = draw_detections_person(im, dets, nboxes, thresh, names, alphabet, l.classes, person_loc);
        ...
        while (only_name[i] != '\0')
        {
            save_img[j] = only_name[i];
            j++;
            i++;
        }
        save_image(im, save_img);
        ...
        while (only_name[i] != '\0')
        {
            save_txt[j] = only_name[i];
            j++;
            i++;
        }
        // save person loc
        FILE* pf = fopen(save_txt, "w");
        fprintf(pf,"# left, right, top, bottom\n");
        for (int xt = 0; xt < person_num; xt++) {
            fprintf(pf, "%d, %d, %d, %d\n", person_loc[xt*4+0], person_loc[xt*4+1], person_loc[xt*4+2], person_loc[xt*4+3]);
        }
        fclose(pf);
...
void run_detector(int argc, char **argv)
{
    ...
    else if(0==strcmp(argv[2], "folder")) test_detector_folder(datacfg, cfg, weights, filename, thresh, hier_thresh, outfile, fullscreen);
